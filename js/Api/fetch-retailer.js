    ...
    chrome.storage.local.get(['accessToken'], function (result) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                const retailerItems = data.retailers.reduce((all, item) => `${all}<a href="${item.link}" target="_blank" class="store-item"><img id=${item.name} src="http://ex.travelcast.us/${item.logoUrl}"></a>`, '');
